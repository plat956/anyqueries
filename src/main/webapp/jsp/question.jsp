...
<div class="dx-blog-post dx-ticket dx-ticket-open">
    ...
    <div style="background-color: #fafafa;">
        <ul class="dx-blog-post-info dx-blog-post-info-style-2 mb-0 mt-0">
            <li><span><span class="dx-blog-post-info-title"><fmt:message
                    key="label.category"/></span>${fn:escapeXml(question.category.name)}</span></li>
            <li><span><span class="dx-blog-post-info-title"><fmt:message key="label.status"/></span>
                <label class="form-check-label" for="status">
                    <span class="badge badge-${question.closed ? 'secondary' : 'success'} user-role-span"
                          style="font-size: 0.9em"><fmt:message
                            key="label.status.${question.closed ? 'closed' : 'open'}"/></span>
                </label>
                <c:if test="${!empty principal && principal.id == question.author.id}">
                    <label class="switch" style="margin-top: -5px;">
                        <input type="checkbox" class="form-check-input" data-toggle="switchbutton" name="status"
                               id="status"
                               onchange="questions.changeStatus('${pageContext.request.contextPath}', this, ${question.id});"${question.closed ? ' checked' : ''}>
            ...
            <li><span><span class="dx-blog-post-info-title"><fmt:message key="label.modified"/></span>
                    <at:time-duration
                            date="${!empty question.editingDate ? question.editingDate : question.creationDate}"/>
                </span></li>
        </ul>
    </div>
    <div class="dx-separator"></div>
        ...
        <div>
            <div>
                <img class="dx-comment-img" src="
                    ...
                    " alt="">
                <span class="badge badge-${question.author.role.color} user-role-span"
            ...
            <div class="dx-comment-cont">
                <a href="#"
                   onclick="questions.showProfile('${pageContext.request.contextPath}', ${question.author.id}, event); return false;"
                   class="dx-comment-name">${fn:escapeXml(question.author.fio)}</a>
                ...
                <span id="dropdownMenuButton${a.id}" data-toggle="dropdown">
                        <c:if test="${!empty principal.id}">
                            ...
                            <c:set var="delete_access"
                                   value="${principal.role == 'ADMIN' || principal.role == 'MODERATOR' || principal.id == question.author.id}"/>
                            <c:if test="${edit_access || delete_access}">
                                <a class="edit-answer-btn" data-toggle="tooltip" data-placement="right"
                                   title="<fmt:message key="label.question.management" />">
                                    <i class="fas fa-cog"></i>
                                </a>
                                <div class="dropdown-menu answer-dropdown"
                                     aria-labelledby="dropdownMenuButton${question.id}">
                                    <c:if test="${edit_access}">
                                        <a class="dropdown-item drop-lnk"
                                           onclick="location.href = '${pageContext.request.contextPath}/controller?command=edit_question_page&id=${question.id}'"><i
                                                class="fa fa-edit" aria-hidden="true"
                                                style="color: #007bff"></i> <fmt:message key="label.edit"/></a>
                                    </c:if>
                                    <c:if test="${delete_access}">
                                        <a class="dropdown-item drop-lnk"
                                           onclick="questions.delete(event, '${pageContext.request.contextPath}', ${question.id}, true)"><i
                                                class="fa fa-trash" aria-hidden="true"
                                                style="color: red"></i> <fmt:message key="label.delete"/></a>
                                    </c:if>
                                </div>
                            </c:if>
                        </c:if>
                    </span>
                <div class="dx-comment-date"><at:time-duration date="${question.creationDate}"/><c:if
                        test="${!empty question.editingDate}"> (<fmt:message key="label.edit.short"/>. <at:time-duration
                        date="${question.editingDate}"/>)</c:if></div>
                ...
                <ul class="attachments download" style="margin-top:10px;margin-bottom: 0px;">
                    <c:forEach var="a" items="${attachments}">
                        <li onclick="questions.downloadAttachment('${pageContext.request.contextPath}/controller?command=download&file=${a.file}')">
                                <span>
                                    <i class="fa fa-file file-attachment file-i"
                                       aria-hidden="true"></i>${fn:escapeXml(a.file)}
                                </span>
                        </li>
                    </c:forEach>
                </ul>
                <c:if test="${!question.closed && !empty principal && question.author.id != principal.id && empty editParam}">
                    <p></p>
                    <a class="reply-link" onclick="dataForms.reply('${fn:escapeXml(question.author.fio)}');"><i
                            class="fa fa-reply" aria-hidden="true"></i> <fmt:message key="label.reply.button"/></a>
                </c:if>
            </div>
        </div>
    </div>
        ...
        <div class="dx-comment dx-ticket-comment" id="answer-box-${a.id}">
            <div>
                <div>
                    ...
                    " alt="">
                    <span class="badge badge-${a.author.role.color} user-role-span"
                          style="display: table;margin: 0 auto;margin-top:5px">
                ...
                <div class="dx-comment-cont">
                    <a href="#"
                       onclick="questions.showProfile('${pageContext.request.contextPath}', ${a.author.id}, event); return false;"
                       class="dx-comment-name">${fn:escapeXml(a.author.fio)}</a>
                    <span id="dropdownMenuButton${a.id}" data-toggle="dropdown">
                                <c:if test="${!empty principal.id}">
                                    <c:set var="edit_access"
                                           value="${!question.closed && principal.id == a.author.id}"/>
                                    <c:set var="delete_access"
                                           value="${principal.role == 'ADMIN' || principal.role == 'MODERATOR' || principal.id == a.author.id}"/>
                                    <c:if test="${edit_access || delete_access}">
                                        <a class="edit-answer-btn" data-toggle="tooltip" data-placement="right"
                                           title="<fmt:message key="label.answer.management" />">
                                            <i class="fas fa-cog"></i>
                                        </a>
                                        <div class="dropdown-menu answer-dropdown"
                                            ...
                                            <c:if test="${edit_access && editParam != a.id}">
                                                <a class="dropdown-item drop-lnk"
                                                   onclick="location.href = '<at:query-parameter-changer key="edit"
                                                                                                         value="${a.id}"/>'"><i
                                                        class="fa fa-edit" aria-hidden="true"
                                                        style="color: #007bff"></i> <fmt:message key="label.edit"/></a>
                                            </c:if>
                                            <c:if test="${delete_access}">
                                                <a class="dropdown-item drop-lnk"
                                                   onclick="answers.delete(event, '${pageContext.request.contextPath}', ${a.id})"><i
                                                        class="fa fa-trash" aria-hidden="true"
                                                        style="color: red"></i> <fmt:message key="label.delete"/></a>
                                            </c:if>
                                        </div>
                    ...
                    <div class="dx-comment-date"><at:time-duration date="${a.creationDate}"/><c:if
                            test="${!empty a.editingDate}"> (<fmt:message key="label.edit.short"/>. <at:time-duration
                            date="${a.editingDate}"/>)</c:if></div>
                            ...
                            <ul class="attachments download" style="margin-top:10px;margin-bottom: 10px;">
                                <c:forEach var="at" items="${a.attachments}">
                                    <li onclick="questions.downloadAttachment('${pageContext.request.contextPath}/controller?command=download&file=${at.file}')">
                                            <span>
                                                <i class="fa fa-file file-attachment file-i"
                                                   aria-hidden="true"></i>${fn:escapeXml(at.file)}
                                            </span>
                                    </li>
                                </c:forEach>
                            </ul>
                                ...
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <c:if test="${!empty principal && principal.id != a.author.id}">
                                            <a id="unlike_${a.id}"
                                               onclick="answers.like('${pageContext.request.contextPath}', ${a.id}, false);"
                                               class="like-none"><i class="far fa-thumbs-down"></i></a>
                                        </c:if>
                                    </c:otherwise>
                                </c:choose>
                                <span class="badge badge-${rating_color}" style="cursor: default"
                                      id="rating_${a.id}">${a.rating}</span>
                                <c:choose>
                                    <c:when test="${a.currentUserGrade > 0}">
                                    ...
                                    <c:otherwise>
                                        <c:if test="${!empty principal && principal.id != a.author.id}">
                                            <a id="like_${a.id}"
                                               onclick="answers.like('${pageContext.request.contextPath}', ${a.id}, true);"
                                               class="like-none"><i class="far fa-thumbs-up"></i></a>
                                        </c:if>
                                    </c:otherwise>
                                </c:choose>
                            </div>
                    ...
                    <c:if test="${editParam == a.id && !question.closed}">
                        <div id="comment-edit-data-${a.id}">
                            <form class="needs-validation" method="post" enctype="multipart/form-data"
                                  action="${pageContext.request.contextPath}/controller?command=edit_answer" novalidate
                                  autocomplete="off">
                                <input type="hidden" value="${a.id}" name="id"/>
                                <textarea
                                        class="summernote form-control<at:field-class-detector field="${validationResult.getField(RequestParameter.TEXT)}" />"
                                        name="text" id="answer_text${a.id}"
                                        maxlength="${AppProperty.APP_ANSWER_MAXLENGTH}" required><c:out
                                        value="${!empty validationResult ? validationResult.getValue(RequestParameter.TEXT) : a.text}"/></textarea>
                                ...
                                <fmt:message key="label.textarea.remaining"/> <span
                                    id="answer_text${a.id}-counter">${AppProperty.APP_ANSWER_MAXLENGTH}</span>
                                <div class="form-group">
                                    <label class="" for="file-selector${a.id}" data-toggle="popover"
                                           data-trigger="hover" data-placement="bottom"
                                           style="cursor:pointer;height: 40px;margin-bottom: 0px;"
                                        ...
                                        <input id="file-selector${a.id}" type="file" multiple name="file"
                                               style="display:none">
                                        <div style="margin: 10px 10px -10px 0px;">
                                            <i class="fa fa-paperclip" aria-hidden="true"></i> <fmt:message
                                                key="label.uploadAttachments"/>
                                        </div>
                                    </label>
                                    <ul class="attachments" id="attachments-list${a.id}" style="margin-top: 10px">
                                        <c:forEach var="c" items="${a.attachments}">
                                            <li>
                                                        <span>
                                                            <i class="fa fa-file file-attachment file-i"
                                                               aria-hidden="true"></i>${fn:escapeXml(c.file)}
                                                        </span>
                                            </li>
                                        </c:forEach>
                                    </ul>
                ...
                <c:if test="${!question.closed}">
                    <c:if test="${!empty principal && a.author.id != principal.id && empty editParam}">
                        <a class="reply-link" id="reply-link${a.id}"
                           onclick="dataForms.reply('${fn:escapeXml(a.author.fio)}');"><i class="fa fa-reply"
                                                                                          aria-hidden="true"></i>
                            <fmt:message key="label.reply.button"/></a>
                    </c:if>
                </c:if>
                <c:if test="${a.solution || (!empty principal && principal.id == question.author.id && !question.closed)}">
                    <a class="solution-link<c:if test="${a.solution}"> solution-mark</c:if>" id="solution_${a.id}"
                            <c:if test="${!question.closed && !empty principal && principal.id == question.author.id}">
                                ...
                                data-toggle="tooltip" data-placement="right" title="<fmt:message
                                    key="label.solution${a.solution ? '.remove' : ''}"/>"
                                data-def-title="<fmt:message key="label.solution"/>" data-rm-title="<fmt:message
                                    key="label.solution.remove"/>"
        ...
        <div class="dx-comment dx-ticket-comment dx-comment-replied dx-comment-new" style="margin-bottom: -20px;"
            ...
            <div>
                ...
                <div class="dx-comment-cont" style="width: 100%;">
                    <form id="answerForm" class="needs-validation" method="post" enctype="multipart/form-data"
                          action="${pageContext.request.contextPath}/controller?command=create_answer" novalidate
                          autocomplete="off">
                        <input type="hidden" value="${question.id}" name="question"/>
                        <h3 class="h6 mb-10"><fmt:message key="label.reply"/></h3>
                        <textarea
                                class="summernote form-control<at:field-class-detector field="${validationResult.getField(RequestParameter.TEXT)}" />"
                                name="text" id="text" maxlength="${AppProperty.APP_ANSWER_MAXLENGTH}" required><c:out
                                value="${validationResult.getValue(RequestParameter.TEXT)}"/></textarea>
                        <c:if test="${!empty validationResult.getMessage(RequestParameter.TEXT)}">
                            <div class="invalid-feedback-backend">
                                <fmt:message key="${validationResult.getMessage(RequestParameter.TEXT)}"/>
                            </div>
                        ...
                        <fmt:message key="label.textarea.remaining"/> <span
                            id="text-counter">${AppProperty.APP_ANSWER_MAXLENGTH}</span>
                        <div class="form-group">
                            <label class="" for="file-selector" data-toggle="popover" data-trigger="hover"
                                   data-placement="bottom" style="cursor:pointer;height: 40px;margin-bottom: 0px;"
                                   data-content="<fmt:message key="message.attachment.info.part1" /> ${AppProperty.APP_ATTACHMENT_COUNT}. <fmt:message key="message.attachment.info.part2" /> ${AppProperty.APP_ATTACHMENT_SIZE} <fmt:message key="message.mb" />">
